...
class AddOptionsMealGuideView: UIView {
    ...
    let label: UILabel = {
        ...
        label.text = "Adding meal function enables you to create a meal that contains several food items and then eat the meal. You can also eat an exsiting meal that is created before. "
    ...
    override init(frame: CGRect) {
        ...
        label.topAnchor.constraint(equalTo: topAnchor, constant: 64 + frame.height*0.4).isActive = true
        ...
        //                    endAngle: 2.0 * .pi,
        //                    clockwise: false)
        path.addRects([CGRect(x: 0, y: 64 + frame.height*0.21, width: frame.width, height: frame.height*0.15),CGRect(x: 0, y: 0, width: frame.width, height: frame.height)])
